...
// ── 배치 규칙 ────────────────────────────────────────────────────────
//
// 섹션은 성향 7축 그대로 7개. 직무능력 15문항은 일곱 섹션에 나눠 섞는다(07).
// 뒤에 몰아두면 "이제 능력 평가구나" 하고 태도가 바뀌기 때문이다.
//
// **같은 능력의 문항은 서로 다른 섹션에 흩어놓는다.** 나란히 두면 앞
// 문항의 답을 보고 뒤를 맞추게 된다 — 특히 형성적 지표는 문항끼리
// 달라야 하는데 붙여 두면 「같은 걸 또 묻네」로 읽힌다.
//
// 2026-08-25 축당 3 → 5문항. 섹션마다 둘씩, 마지막 섹션만 셋.
const ABILITY_PLACEMENT: Record<number, string[]> = {
  1: ["AB-COOP-01", "AB-ORG-04"],
  2: ["AB-ORG-01", "AB-AUTO-04"],
  3: ["AB-AUTO-01", "AB-COOP-04"],
  4: ["AB-COOP-02", "AB-ORG-02"],
  5: ["AB-AUTO-02", "AB-ORG-05"],
  6: ["AB-COOP-03", "AB-ORG-03"],
  7: ["AB-AUTO-03", "AB-COOP-05", "AB-AUTO-05"],
};

/** 섹션 안에서 직무능력 문항이 들어갈 자리 (0부터 센다) */
const ABILITY_SLOTS = [4, 11, 14];
