...
import { scoreAssessment, type Answer, type ScorableItem } from "../scoring/score";
import { ABILITY_AXIS_FROM_DB } from "../items/types";
import { assessQuality, type AntonymPair, type ResponseRecord } from "../scoring/quality";
import {
  SECTION_COUNT,
...
export async function submitAction(sessionId: string) {
  ...
  if (remaining !== null)
    return { error: `아직 답하지 않은 문항이 있습니다 (${remaining}번째 묶음)` };
