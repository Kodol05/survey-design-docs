  ...
  return (
    <section className="border-t border-[--border] py-10">
      <div className="mb-6 flex items-baseline gap-3">
        ...
        <span className="tabular text-lg">{Math.round(percent)}</span>
        <span className="text-ink-muted text-table">{BAND_LABEL[band]}</span>
      </div>
      ...
      <div className="grid gap-10 lg:grid-cols-[minmax(0,1fr)_22rem]">
        <div>
          ...
          <div className="flex items-start gap-5">
            <span className="w-10 shrink-0 pt-1 text-right font-medium" style={{ color: LOW_INK }}>
              낮음
            </span>
            ...
            <span className="w-10 shrink-0 pt-1 font-medium" style={{ color: HIGH_INK }}>
              높음
            </span>
          </div>

          {poles && (
            <div className="text-axis text-ink-muted mt-3 flex gap-8">
              <p className="flex-1">{poles.low}</p>
              <p className="flex-1 text-right">{poles.high}</p>
            </div>
          )}

          <p className="mt-6 leading-relaxed">{BAND_TEXT[scale]?.[band]}</p>
        </div>

        {/* 하위척도 — 실제 결과지 2페이지에 해당 */}
        {facets.length > 0 && (
          <div>
            <p className="text-axis text-ink-muted mb-3">이 축을 이루는 것</p>
