...
> 새 다이어그램 종류를 쓰고 싶으면, 먼저 아무 문서에나 `info` 블록을 하나 넣어 사내 GitHub의 Mermaid 버전을 확인하세요.
>
> ⚠️ 예전에는 `docs/02_다이어그램_샘플.md`를 가리켰는데, **그 파일은 파일명을 ASCII로 바꾸면서 없어졌습니다.** 한글 파일명이 GitHub 주소에서 깨져 공유가 안 됐기 때문입니다.
...
(예전 샘플 문서 6번이 이 방식이었습니다. 그 파일은 지금 없습니다.)
