...
들어오는 문은 하나입니다. 구성원은 이름과 전화번호를 넣고 바로 시작하고, 관리자는 같은 화면에 있는 관리자 로그인 버튼을 눌러 아이디와 비밀번호로 들어갑니다. 관리자 계정은 미리 만들어둔 것만 쓸 수 있습니다.
...
여기에 더해 응답이 성실했는지를 보는 지표를 함께 계산합니다. 문항당 응답 시간이 지나치게 짧거나, 서로 반대여야 정상인 문항 쌍에 같은 방향으로 답한 경우를 잡아냅니다. 그 사람의 성격을 판정하는 게 아니라 이 데이터를 믿어도 되는지를 보는 것입니다.
...
**대시보드**에서는 응시를 마친 인원과 진행 중인 인원을 먼저 봅니다. 그 아래에 회사 전체의 성향 분포가 나오는데, 축별 평균과 함께 응답이 얼마나 퍼져 있는지도 보여줍니다. 평균이 비슷해도 어떤 축은 다들 비슷하게 답하고 어떤 축은 사람마다 극과 극일 수 있습니다.
...
특정 구성원의 응시를 초기화할 수도 있습니다. 응답이 정상적으로 이뤄지지 않았을 때 다시 받게 하는 용도입니다.
...
소요 시간과 문항 수, 중간에 그만두어도 이어서 할 수 있다는 점을 먼저 알려주고, 그 아래에 설문의 성격을 적습니다. 클로닝거의 기질·성격 이론을 참고해 사내에서 만든 도구이고 일곱 개 축으로 성향을 나눠 본다는 것, 정답과 오답이 없다는 것 정도입니다. 안내가 길어지면 오히려 눈에 안 들어와서 짧게 씁니다.

### 응시 화면

105문항을 한 화면에 늘어놓으면 끝까지 집중하기 어렵습니다. 몇 가지 장치를 넣었습니다.
...
맨 위에 응시를 마친 인원과 진행 중인 인원이 나옵니다. 계정이 로그인할 때 만들어지는 구조라 아직 들어오지 않은 사람은 집계에 잡히지 않습니다. 전 직원 대비 비율이 아니라 지금까지 마친 사람 수를 보는 숫자입니다.
...
**훑어보기**는 이름·상태·응시일과 함께 일곱 축 점수를 작은 막대로 보여줍니다. 숫자만 늘어놓는 것보다 막대로 보는 편이 패턴을 훨씬 빨리 잡습니다. 축 이름을 누르면 그 축 기준으로 정렬되어 위쪽이 곧 그 성향이 두드러진 사람들입니다.
...
현재 나가는 문항을 구간별로 묶어 보여줍니다. 문항 내용과 어느 축인지, 방향이 반대인지, 어떤 자료를 근거로 만들었는지가 함께 나옵니다. 마지막 항목이 이 화면을 두는 이유입니다. 저작권 대응이 말로만 끝나지 않으려면 문항이 어디서 나왔는지 언제든 확인할 수 있어야 합니다. 읽기 전용이고, 문항 수정은 파일에서 합니다.
...
**구성원 삭제.** 퇴사자나 오타로 만들어진 계정을 정리합니다. 계정과 응답, 결과, 이력이 전부 지워지고 되돌릴 수 없어서 확인 창에 대상자 이름을 직접 입력해야 실행됩니다. 관리자 계정은 삭제할 수 없게 막았습니다. 마지막 관리자를 지우면 아무도 들어갈 수 없게 됩니다.
...
전화번호는 `010-1234-5678`과 `01012345678`을 같은 것으로 처리합니다. 이걸 빠뜨리면 하이픈 유무 때문에 같은 사람이 못 들어오는 일이 생깁니다. 같은 이름과 번호로 5분 안에 열 번 실패하면 5분 동안 잠기는데, 사무실이 인터넷 주소를 함께 쓰므로 이름과 번호 조합 단위로 막습니다.
